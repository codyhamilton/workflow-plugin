...
- Contract: `execute` always stops at its closing commit and reports; it never dispatches the next phase itself. Whether another phase runs next is not a posture `execute` checks or a flag declared to it — it is a property of the invocation, exactly as under the driver (Domain: Driver, Transport): a bare `execute` call has nothing dispatching a successor and simply stops; a **coordinator** — the context that invoked the first `execute` and has done no phase work, instructed to run the design through completion — dispatches `execute` for each phase in series as a subagent with the declared review posture, holding nothing but each report, and repeats on `closed`. On anything else (`incomplete` after its one re-dispatch, or `unsuccessful`) it stops and relays the report, unresolved. The coordinator does not resolve state, read artifacts, or restate briefs.
  - **Nesting depth required: two levels**, not just "some." One for coordinator → `execute` per phase, one for `execute` → its own `refine`/workers inside that phase. A harness with exactly one level of subagent dispatch runs a single `execute` invocation fine — `execute` still gets its one level for workers — but cannot also wrap it in a coordinator, because the coordinator's dispatch would consume the only level `execute` needs for its own. A harness with zero levels cannot run `execute` at all. Either way, an unattended, single-session, multi-phase run is not this path's job on such a harness: flattening the coordinator's role into the top context (calling `refine`/workers directly, per phase, in series) recreates the accumulating orchestrator this design exists to avoid — it just moved up one frame. That case routes to the driver (Domain: Driver), a separate process rather than a nested subagent, so it is not subject to the harness's nesting depth at all.
- Non-goals: parity with the driver; provider selection; cost measurement; a `hold`/`continue` posture — see Architectural Implications; an unattended multi-phase run on a harness with fewer than two levels of subagent nesting — that is the driver's case, not this one.
...
- **Naive coordination needs two levels of subagent nesting, not just "some."** One for a coordinator to dispatch `execute` per phase, one for `execute` to dispatch `refine`/workers inside that phase. A harness with exactly one level runs a single `execute` invocation fine but cannot also wrap it in a coordinator without exhausting the level `execute` needs for its own workers. An unattended, multi-phase run on such a harness is out of scope for naive coordination — flattening the coordinator's role into the top context (dispatching `refine`/workers directly, per phase) recreates the accumulating orchestrator this design exists to avoid. Routes to the driver instead, a separate process unaffected by the harness's nesting depth.
- **The driver should eventually run external to its caller, not co-located.** Direction only, not built now: today the CLI runs wherever it's invoked, and the MCP front end's phase agent runs on the calling session's own host, so an MCP caller's container dying still orphans the dispatch (Open Questions). Moving execution to its own, longer-lived host removes that coupling entirely rather than mitigating it. Out of scope until the container-lifetime question is researched; Phase 4 ships MCP co-located with its caller.
...
- MCP host lifetime: a cloud agent's container can be reclaimed mid-phase, orphaning the dispatched phase agent. Local/CLI use avoids this because the host is the invoker's own machine, kept up for the run's duration. Deferred; noted in Phase 4. Eventual direction is an externally-hosted driver, decoupled from the caller's own container (Decisions); not scoped here.

## Phases

### Phase 1 — Closure is deterministic

- Outcome: on a branch carrying plan `<slug>` with phases 1..k closed by `execute`, `git log <default>..HEAD --format='%(trailers:key=Workflow-Phase,valueonly)'` lists exactly `<slug>:1` … `<slug>:k`, each on the commit that closed that phase's record in `IMPLEMENTATION.md`, and after the after-last-phase path `<slug>:done` on the commit that ended it; `execute` stops at its closing commit and reports on every invocation, never dispatching a successor itself; a coordinator instructed to run the design to completion, in a harness with two levels of subagent nesting (coordinator → `execute` per phase, `execute` → its own workers), dispatches each phase in series holding only reports; and on a harness with only one level, a single `execute` invocation still works but a wrapping coordinator does not — an unattended multi-phase run there is out of scope for this path and is named as such, not silently flattened into the top context.
...
- **Why naive coordination needs two levels of nesting, not one.** `execute` already spends a level dispatching `refine`/workers inside a single phase; a coordinator wrapping multiple phases spends a second level dispatching `execute` itself. A harness capped at one level can still run `execute` directly — it only needs its own level — but has nothing left for a wrapping coordinator. The failure mode isn't "no naive coordination" so much as a tempting wrong answer: collapse the coordinator and `execute` into one context and call `refine`/workers directly, per phase. That's still one context accumulating every phase's work, just renamed; it is the orchestrator this design exists to bound, not an exception to it. Naming the threshold precisely (two levels, not "some nesting") makes that wrong answer visible instead of something a naive reading of the old text could paper over.
...
- **Why the driver is the answer for single-level-nesting harnesses, and why it should eventually run external to its caller.** Naive coordination needs two levels of nesting (above); a harness with one level can't supply the second, and the driver isn't a nested subagent at all — it's a separate process, so a harness's dispatch-nesting cap doesn't apply to it. But today that process still runs on whatever host called it: the CLI on the invoker's machine, the MCP server on the calling agent's own container. For a human at a terminal that's fine — the terminal outlives the run. For a cloud agent session driving MCP, it isn't: the same container-lifetime risk that motivated one-phase-per-call (previous note) means the driver, even over MCP, is still coupled to a host it doesn't control. Bounding background spans to one phase is a mitigation, not a fix. The fix is decoupling the driver's execution host from its caller's entirely — a direction recorded in Decisions and Open Questions, not built here, pending research into how long a calling session's own host actually persists with an active background process.
