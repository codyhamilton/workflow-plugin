...
These were settled by the user in the plan conversation. Executors do not re-open them at their own discretion — but they are bets on the hypotheses, not axioms. If a Phase 1 eval falsifies a hypothesis underlying one of them (H3 → decision 1, H5 → decision 6), execution stops and returns to a plan conversation with the user; the finding does not license the executor to silently redesign. This is the same rule execute already applies when a plan no longer aligns.
...
3. **Execute skill revision.** Brief-based worker dispatch (author-once, route-verbatim); progressive IMPLEMENTATION.md during execution (crash-resumable branches); remove `STATUS.md` — partial/paused state must be recoverable from progressive IMPLEMENTATION.md alone, and a status artifact violates the taxonomy's "nothing carries status"; review posture parameter (full independent review when terminal, pre-flight when a downstream review stage exists); delegation conditional on harness capability and context economics; session-ID capture and model tables moved out of the skill body into reference/config files.
...
Phase 1 precedes 2–4 because the revisions are bets on the hypotheses; where an eval needs a candidate variant (briefed dispatch, fused plan-build, commentary-stripped skills), the variant is built as an eval candidate inside Phase 1, not merged first. Phase 1 findings act at two levels: findings *within* the model (e.g. how briefs are best structured) re-scope Phases 2–4 directly; findings that *falsify* a model decision's hypothesis (H3, H5) halt execution and return to the user — see The Model. Phase 5 depends on 2–4.

H1 (verbatim intent), H4 (status in tracker), and H6 (cold reader) are not Phase 1 harness evals: H1 is already informally validated and its harness cost is high; H4 and H6 are artifact-design claims best validated observationally, through the workflow-tuning harvest of real pipeline outcomes once the pipeline runs. They stay in the README as falsifiable, with their validation route named there rather than pretending a scenario run would test them.

## Acceptance Criteria

- README carries a concise operating-hypotheses section; each hypothesis states its empirical status and its validation route (harness eval vs observational).
- `evals/scenarios/` contains at least one scenario for each of H2, H3, and H5, with baseline results recorded before skill revisions merge; each of those hypotheses has a written findings note stating what result would change which decision.
