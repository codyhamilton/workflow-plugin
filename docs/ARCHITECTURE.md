...
```
Workflow Plugin (Cursor / Claude Code / Codex)
├── workflow-protocol.json  → workflow-protocol: 1
├── protocol/
│   ├── artifacts.md        → portable artifact conventions
│   └── models.yaml         → harness-specific model recommendations
├── skills/
│   ├── workflow-plan/      → Creates PLAN.md, DESIGN.md, owns provenance capture
│   ├── workflow-execute/   → Executes plans, writes IMPLEMENTATION.md + REVIEW.md
│   ├── comprehensive-review/  → Independent review of completed work
│   ├── workflow-setup/     → Bootstrap docs/ARCHITECTURE.md + docs/ROADMAP.md
│   ├── iterate/            → Long-horizon divergent plan/execute cycles
│   ├── workflow-tuning/    → Meta-skill: improve workflow itself, run evals
│   └── transcript-parser/  → Cost metrics from session transcripts
├── docs/
...
**workflow-plan**: Single responsibility is capturing intent and producing durable plan artifacts.
...
**workflow-execute**: Single responsibility is orchestrating implementation of an existing plan.
...
- Model picks come from protocol/models.yaml, not from artifact rules
...
**workflow-setup**: Single responsibility is bootstrapping missing stable docs.
...
- Does NOT plan or execute; hooks into workflow-plan afterward
...
7. Artifact conventions and model recommendations stay in separate protocol files

## Data Flows

**Planning session**:
1. User → workflow-plan: "I want to <request>"
...
**Execution session**:
1. User → workflow-execute: "Execute <plan>"
...
**Setup session**:
1. User → workflow-setup: "Set up architecture docs"
...
**Eval session** (meta):
1. User → workflow-tuning: "Eval plan on <fixtures>"
...
- [workflow-protocol.json](../workflow-protocol.json) — machine-readable protocol version
- [protocol/artifacts.md](../protocol/artifacts.md) — portable artifact rules
- [protocol/models.yaml](../protocol/models.yaml) — model recommendations
- [skills/workflow-plan/SKILL.md](../skills/workflow-plan/SKILL.md) — plan skill full prompt
- [skills/workflow-plan/templates/PLAN.md](../skills/workflow-plan/templates/PLAN.md) — plan template
