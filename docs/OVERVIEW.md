...
3. **comprehensive-review** — Independent review keyed to `PLAN.md`'s acceptance criteria. Writes `REVIEW.md` (with a machine-actionable verdict) and per-finding remediation briefs into the plan folder; includes an intent-and-assumptions lens and a plan-sufficiency judgment. Falls back to a recovered-intent note when a functional/material PR arrives with no plan folder; trivial non-functional work may skip that ceremony.
4. **post-build** — The pipeline stage that picks a PR up where the build leaves off: classifies the change and right-sizes the process, independent review via `comprehensive-review` when needed, bounded finding-scoped remediation with a fresh re-review, reactive wait for required checks, conditional `QA.md` / exact-SHA deploy / browser QA for functional driveable need, and an uncommitted merge-readiness report. Repo mechanics come from a per-repo adapter skill.
