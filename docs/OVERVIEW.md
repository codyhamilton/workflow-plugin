...
5. **close-out** — Ends a plan. Collapses the entire plan folder into one record file, `docs/plans/<NN>-<slug>.md`, recording what was actually built (a section per phase when there was more than one), deviations and why, how review and QA resolved, residual risks, and where follow-ups went; promotes a `DESIGN.md` worth keeping to `docs/design/`; deletes the folder in one commit. The branch history and the PR are the archive; the record file is the record.
...
- Every plan ends. One durable record per change: a single file, `docs/plans/<NN>-<slug>.md`, that replaces the whole plan folder. The branch history and the PR are the archive.
