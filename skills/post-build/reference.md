...
An orchestrator that composes each worker's prompt pays for the instruction text twice — once reading whatever it derives the prompt from, once emitting the prompt — and the instructions drift, because each run re-invents phrasing, drops a rule, or paraphrases the skill it should be routing. The split fixes both: **static direction** lives in a versioned brief beside the skill (`briefs/`) that only the worker ever reads, and **dynamic context** — the few lines only the run knows (paths, SHAs, URLs, finding IDs) — is the entire authored surface of the dispatch. The full instruction text never enters the orchestrator's context at all; instruction review happens in the plugin repo; every run dispatches the same discipline. The reviewer needs no standing brief because `comprehensive-review` — a full skill — already is one. Per-finding remediation briefs are the deliberate exception: they *are* dynamic context, authored by the reviewer into the plan folder because only the reviewer holds that finding's context — and the same rule still applies, since the orchestrator routes them by path, never by restating them. The standing `remediation-fixer.md` carries only the discipline that never changes between findings.
