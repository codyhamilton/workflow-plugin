...
- Findings reach a fixing agent as an orchestrator's paraphrase instead of the reviewer's own brief, and worker prompts are re-derived long-hand each run instead of routing the standing briefs.
...
- **Static direction** — a standing brief in this skill's own `briefs/` directory, routed to the worker by path. The worker reads it; you do not. Do not open, inline, or paraphrase a worker brief into a prompt — the point of the split is that the instruction text never enters your context at all. Independent review is the exception: it is a full skill of its own, named rather than pathed.
- **Dynamic context** — the few situational lines only this run knows: repository and branch, plan folder, SHAs, proven URL, finding IDs and remediation-brief paths, adapter specifics. You write exactly this part, and nothing more.

A dispatch therefore reads: "Read `<absolute path to the brief>` — it is your task discipline. Situational context: …".

**Resolve brief paths against this skill's own directory** and pass the absolute path. You know where this file was loaded from; the worker does not, and the plugin's install location differs between a marketplace cache and a bare skills directory. A relative path in a dispatch is a path the worker cannot open.

| Phase | Static direction | Dynamic context |
|-------|------------------|-----------------|
| Independent review | `comprehensive-review` skill (named, not pathed) | PR, plan folder or no-plan note |
| Remediation fix | `briefs/fixer.md` | branch, plan folder, path to the reviewer's per-finding remediation brief (routed by path, never restated) |
| Remediation verification | `briefs/verifier.md` | branch, plan folder, reviewed SHA, new candidate SHA, findings claimed resolved |
| QA planning | `briefs/qa-planner.md` | branch, plan folder, adapter QA environment notes |
| Deployed QA | `briefs/qa-driver.md` | proven URL, candidate SHA, `QA.md` path, adapter identities/setup |
...
3. **Remediation** (only on `REMEDIATE`). Dispatch a fixer (`briefs/fixer.md`) per accepted briefed finding, with dynamic context naming the branch, plan folder, and the path to the reviewer's remediation brief (routed by path, never restated). The fixer touches only its finding, runs the brief's done evidence, and records the resolution in `REVIEW.md` without erasing the original finding.
4. **Verification** (only when step 3 ran). Dispatch a fresh verifier (`briefs/verifier.md`) — not the reviewer, not a fixer — to verify the remediation delta and update `REVIEW.md`'s verdict for the new SHA. One remediation cycle, one verification: if a `blocker` or `high` finding still stands, stop and hand off. Fixes the reviewer applied in place during step 2 do not trigger this step.
5. **QA planning** (only when QA is applicable — functional/mixed surface with at least one driveable user-facing criterion or review residual that needs a live UI). Dispatch a QA planner (`briefs/qa-planner.md`): it derives `QA.md` from acceptance criteria and `REVIEW.md`'s residual risks and commits it now, before the candidate SHA exists. When QA is not applicable, record `QA: SKIPPED (<reason>)` — do not invent a matrix for undriveable-only or non-functional work.
...
- Worker prompts carry an absolute path to the worker's standing brief plus orchestrator-written dynamic context; never open, template, or restate a brief's instructions.
