...
description: Worker skill for the post-build stage — verify a remediation delta against the findings it claims to resolve and update REVIEW.md's verdict for the new SHA. Load only when dispatched as the fresh verifier by the post-build orchestrator after delegated remediation; not a general review skill.
---
...
This skill is your standing discipline; your dispatch message carries the situational context — the repository and PR branch, the plan folder, the previously reviewed SHA, the new candidate SHA, and which findings are claimed resolved.
